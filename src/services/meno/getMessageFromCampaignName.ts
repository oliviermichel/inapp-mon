...
export async function getMessageFromCampaignName(campaignName: string): Promise<FeedMessage | null> {
    await refreshAccessToken();
    
    const baseUrl = process.env.MENO_BASE_URL;
    const messagesUrl = process.env.MENO_MESSAGE_ENDPOINT;

    const url = `${baseUrl}${messagesUrl}?filter[messageType]=feed-messages&filter[campaignId]=${campaignName}`;
